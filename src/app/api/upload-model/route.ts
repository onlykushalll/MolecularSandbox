...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!safeName.endsWith('.glb')) return NextResponse.json({ error: 'File must be .glb' }, { status: 400 })
    const bytes = await file.arrayBuffer()
    const buffer = Buffer.from(bytes)
    if (buffer.length < 4 || buffer.toString('ascii', 0, 4) !== 'glTF') return NextResponse.json({ error: 'Not a valid GLB file' }, { status: 400 })
    const modelDir = join(process.cwd(), 'public', 'models')
    await mkdir(modelDir, { recursive: true })
    await writeFile(join(modelDir, safeName), buffer)
    return NextResponse.json({ ok: true, filename: safeName, size: buffer.length, sizeKB: Math.round(buffer.length / 1024) })
  } catch (err) {
